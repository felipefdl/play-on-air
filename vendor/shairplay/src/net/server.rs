...
pub(crate) struct HttpServer {
    ...
    /// Accept-loop task abort handles (one per bound address).
    accept_aborts: Vec<tokio::task::AbortHandle>,
    /// Live `process_connection` tasks — aborted on `stop()` so RTSP TCP dies.
    conn_aborts: Arc<std::sync::Mutex<Vec<tokio::task::AbortHandle>>>,
}
...
impl HttpServer {
    ...
    pub(crate) fn new(callbacks: Arc<dyn HttpdCallbacks>, max_connections: usize) -> Self {
        Self {
            ...
            accept_aborts: Vec::new(),
            conn_aborts: Arc::new(std::sync::Mutex::new(Vec::new())),
        }
    }
    ...
    pub(crate) async fn start(&mut self, port: u16) -> Result<u16, NetworkError> {
        ...
        let conn_aborts = Arc::clone(&self.conn_aborts);
        self.accept_aborts.clear();

        for listener in listeners {
            if let Ok(addr) = listener.local_addr() {
                tracing::debug!(%addr, "Listener bound");
            }
            let abort = spawn_accept_loop(
                listener,
                callbacks.clone(),
                semaphore.clone(),
                shutdown_rx.clone(),
                Arc::clone(&conn_aborts),
            );
            self.accept_aborts.push(abort);
        }
    ...
    /// Stop the server: signal accept loops, abort all live connections, drop listeners.
    ///
    /// PlayOnAir relies on this to force-drop iOS RTSP after Cast steal. Without aborting
    /// `process_connection` tasks, accepted TCP streams stay ESTAB and Now Playing remains.
    pub(crate) async fn stop(&mut self) {
        if let Some(tx) = self.shutdown_tx.take() {
            let _ = tx.send(true);
        }
        for h in self.accept_aborts.drain(..) {
            h.abort();
        }
        if let Ok(mut guard) = self.conn_aborts.lock() {
            for h in guard.drain(..) {
                h.abort();
            }
        }
        self.running = false;
        // Let aborted tasks drop streams (close TCP) before return.
        ...
        loop {
            tokio::select! {
                result = listener.accept() => {
                    ...
                    let aborts = Arc::clone(&conn_aborts);
