...
// Vendored fork for PlayOnAir: path dep is not a workspace member, but
// `cargo clippy --workspace -D warnings` still typechecks it as a dependency.
// Quiet upstream-style noise only — do not weaken play-on-air workspace lints.
